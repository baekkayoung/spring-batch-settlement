...
public class SettlementJobConfig {
    ...
    @Bean
    @StepScope
    public JpaPagingItemReader<Orders> ordersReader(@Value("#{jobParameter['targetDate']}")String targetDate) {
    ...
    @Bean
    public ItemProcessor<Orders, Settlement> settlementProcessor(){ // <Input, Output>
        return item -> {
            int fee = (int) (item.getAmount() * 0.03); // 수수료 3% 계산
            int settlementAmount = item.getAmount() - fee; // 정산금액 계산
